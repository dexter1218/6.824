...
type Clerk struct {
	sm       *shardctrler.Clerk             // 通过它向shardctrler发起请求
	config   shardctrler.Config             // client所知的配置
...
//
// masters[] is needed to call shardctrler.MakeClerk().
//
...
func MakeClerk(masters []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *Clerk {
	ck := new(Clerk)
	ck.sm = shardctrler.MakeClerk(masters) // 关联shardctrler
	ck.make_end = make_end
	// You'll have to add code here.
	ck.config = ck.sm.Query(-1) // 向shardctrler询问最新的配置
	ck.clientId = nrand()
	ck.commandNum = 1
	return ck
}
