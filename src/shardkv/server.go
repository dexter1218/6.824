...
type ShardKV struct {
	...
	mck                   *shardctrler.Clerk              // 关联shardctrler
		...
		if applyMsg.CommandValid { // 如果取出的是command则执行
			...
			if !ok {                        // 断言失败也不报panic，继续执行后面的
				...
				if op.OpType == Get || op.OpType == Put || op.OpType == Append {
					kv.executeClientCmd(op, applyMsg.CommandIndex, 0)
...
// ShardKV定期询问shardctrler以获取更新的配置信息
...
func (kv *ShardKV) getLatestConfig() {
	for !kv.killed() {
		...
		nextConfig := kv.mck.Query(curConfig.Num + 1) // 联系shardctrler获取当前config的下一个config（为了确保每个config变更都执行到）
		...
		time.Sleep(time.Millisecond * ConfigCheckInterval) // 每80ms向shardctrler询问一次最新配置
	}
}
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int,
	...
	// 将masters传给shardctrler.MakeClerk()以便向shardctrler发送rpc
	kv.mck = shardctrler.MakeClerk(kv.masters)
	...
	// 开启goroutine定期轮询shardctrler以获取新配置
