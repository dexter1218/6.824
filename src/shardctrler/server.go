...
const RespondTimeout = 500 // ShardCtrler回复client的超时时间，单位：毫秒
...
type ShardCtrler struct {
	...
	// Your data here.
	sessions    map[int64]Session  // 记录ShardCtrler为每个client处理过的上一次请求的结果
	configs     []Config           // indexed by config num
	notifyMapCh map[int]chan Reply // ShardCtrler执行完请求操作后通过chan通知对应的handler方法回复client，key为日志的index
}

// session跟踪为client处理的上一次请求的序列号，以及相关的响应。
// 如果ShardCtrler接收到序列号已经执行过的请求，它会立即响应，而不需要重新执行请求
type Session struct {
	LastSeqNum int    // ShardCtrler为该client处理的上一次请求的序列号
	OpType     string // 上一次处理的操作请求的类型
	Response   Reply  // 对应的响应
}

// 响应的统一结构，用于ShardCtrler在session中保存最新请求的响应
type Reply struct {
	Err    Err
...
// Join handler
func (sm *ShardCtrler) Join(args *JoinArgs, reply *JoinReply) {
	// Your code here.
	sm.mu.Lock()

	// ShardCtrler过滤重复请求
	if args.SeqNum < sm.sessions[args.ClientId].LastSeqNum {
		// 标志这个请求client已经收到正确的回复了，只是这个重发请求到达ShardCtrler太慢了
...
// Leave handler
func (sm *ShardCtrler) Leave(args *LeaveArgs, reply *LeaveReply) {
	// Your code here.
	sm.mu.Lock()
...
// Move handler
func (sm *ShardCtrler) Move(args *MoveArgs, reply *MoveReply) {
	// Your code here.
	sm.mu.Lock()
...
func (sm *ShardCtrler) Query(args *QueryArgs, reply *QueryReply) {
	// Your code here.
...
func (sm *ShardCtrler) createNotifyCh(index int) chan Reply {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
// ShardCtrler回复client后关闭对应index的notifyCh
func (sm *ShardCtrler) closeNotifyCh(index int) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
// 获取最新的Config
// 调用它的位置需要先持有sm.mu
func (sm *ShardCtrler) getLastConfig() Config {
	return sm.configs[len(sm.configs)-1]
}

// ShardCtrler从applyCh中取出client的config变更请求并实际执行对应操作
func (sm *ShardCtrler) applyConfigChange() {
	for !sm.killed() {
		...
		if applyMsg.CommandValid {
			...
			if !ok {                        // 断言失败也不报panic，继续执行后面的
				...
				if exist && op.OpType != Query && op.SeqNum <= sessionRec.LastSeqNum { // 若为重复的Get请求可以重复执行
					...
					case Join:
						...
						DPrintf("ShardCtrler[%v] Join(SeqNum=%v) apply success! Groups = %v, Shards = %v\n",
							sm.me, op.SeqNum, sm.getLastConfig().Groups, sm.getLastConfig().Shards)
					case Leave:
						reply.Err = sm.executeLeave(op)
						DPrintf("ShardCtrler[%v] Leave(SeqNum=%v) apply success! Groups = %v, Shards = %v\n",
							sm.me, op.SeqNum, sm.getLastConfig().Groups, sm.getLastConfig().Shards)
					case Move:
						reply.Err = sm.executeMove(op)
						DPrintf("ShardCtrler[%v] Move(SeqNum=%v) apply success! Groups = %v, Shards = %v\n",
							sm.me, op.SeqNum, sm.getLastConfig().Groups, sm.getLastConfig().Shards)
					case Query:
						reply.Err, reply.Config = sm.executeQuery(op)
						DPrintf("ShardCtrler[%v] Query(SeqNum=%v) apply success! Groups = %v, Shards = %v\n",
							sm.me, op.SeqNum, sm.getLastConfig().Groups, sm.getLastConfig().Shards)
					default:
					...
					if op.OpType != Query { // Query请求的回复就不用放到session了
						...
						DPrintf("ShardCtrler[%d].sessions[%d] = %v\n", sm.me, op.ClientId, session)
					}
				}
			...
			DPrintf("ShardCtrler[%d] get an unexpected ApplyMsg!\n", sm.me)
		}
	}
}
...
// 实际执行Join Group操作
func (sm *ShardCtrler) executeJoin(op Op) Err {
...
// 实际执行Leave Group操作
func (sm *ShardCtrler) executeLeave(op Op) Err {
	lastConfig := sm.getLastConfig()
	newConfig := Config{}
	newConfig.Num = lastConfig.Num + 1
...
// 实际执行Move shard操作
func (sm *ShardCtrler) executeMove(op Op) Err {
	lastConfig := sm.getLastConfig()
	newConfig := Config{}
	newConfig.Num = lastConfig.Num + 1
...
// 实际执行Query Group操作
func (sm *ShardCtrler) executeQuery(op Op) (Err, Config) {
	lastConfig := sm.getLastConfig()
	// 如果查询的配置号为-1或大于已知的最大配置号，则ShardCtrler应回复最新配置
...
func (sm *ShardCtrler) Kill() {
	atomic.StoreInt32(&sm.dead, 1)
	sm.rf.Kill()
	// Your code here, if desired.
}

func (sm *ShardCtrler) killed() bool {
	z := atomic.LoadInt32(&sm.dead)
	return z == 1
}

// needed by shardkv tester
func (sm *ShardCtrler) Raft() *raft.Raft {
	return sm.rf
}
